...
void normalize_weight( mesh3D &obj){
	for (int x=0; x< obj.nb; x++){
		...
		*/
		...
		if (event.type) {
			...
			*/
			...
			clock_t start = clock();
			mult_vm(man_s.mats, man.vertics, man.vertics_trans, man.w, man.nb );
			clock_t end = clock();
