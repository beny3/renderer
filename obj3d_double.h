...
void make_normal(mesh3D *obj){
	vector u, v;
	vector *vertics = obj->vertics;
	obj->normals_f = new vector[obj->nb_f];

	for (int i=0; i < obj->nb_f; i+=3){	
		u = minus(&vertics[obj->face[i+2]], &vertics[obj->face[i+1]]);
		v = minus(&vertics[obj->face[i  ]], &vertics[obj->face[i+1]]);
		normalize(&u);
		normalize(&v);
		obj->normals_f[i/3]= cross(u, v);
	} 
}


void euler(physic *phy, double dt){
	acc(&phy->position, &phy->velocity, dt);
	vector m = phy->moment;
	double theta = normalize(&m);
	phy->angle += theta*dt;
	make_rot4_u(phy->mat.data, &m, phy->angle);
...
int read_mesh3D(mesh3D *obj, const char* name){
	...
	while(fgets(ligne, 256, file) != NULL){
		if (ligne[0]=='v' && ligne[1]==' '){
			sscanf(ligne, "v %lf %lf %lf", &obj->vertics[v].x, &obj->vertics[v].y, &obj->vertics[v].z);
			double r = dot(&obj->vertics[v], &obj->vertics[v]);
			if (r > obj->radius)obj->radius = r;
			obj->normals[v] = vec(0,0,0);
			v++;	
		}
		if (ligne[0]=='v' && ligne[1]=='t'){	
			sscanf(ligne, "vt %lf %lf", &obj->v_text[vt].x, &obj->v_text[vt].y);
			vt++;
		}
		if (ligne[0]=='f' ){	
...
mesh3D make_cylinder(double x, double y, double z, double sy, double  r, int nb_l, int nb_s){
	...
	int k=0;
	double stepY = sy/(nb_s-1);
	for (int i=0; i<nb_s; i++){
		for (int j=0; j<nb_l; j++){
			double theta = (2*PI*j)/nb_l;
			v[k].x = x + cos(theta)*r;
			v[k].z = z + sin(theta)*r;
			v[k].y = y + i*stepY;
			k++;
...
mesh3D make_rect(double x, double y, double z, double sx, double sy, double sz){
	mesh3D rect;
	rect.nb = 8;
	rect.nb_f = 12*3;
